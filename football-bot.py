...
print(f"🤖 Новый бот: @lyressports_bot")
...
def predict_match(match_data):
    ...
    # ПРОГНОЗ НА ИСХОД
    home_strength = home_avg_scored * 1.2 - away_avg_conceded * 0.8 + 0.3  # +0.3 за домашнее поле
...
def generate_match_report(match_id):
    """ФОРМИРУЕТ ОТЧЕТ ПО ЗАВЕРШЕННОМУ МАТЧУ"""
...
*Доступные команды:*
/predict_all - прогнозы на ВСЕ лиги сегодня
/predict_top5 - только топ-5 лиг
/predict_eng - все английские лиги
/reports - отчеты по завершенным матчам
/help - помощь

*Анализируемые лиги:* АПЛ, ЛаЛига, Бундеслига, Серия А, РПЛ, Бразилия, Чемпионшип
    """
...
📊 Бот анализирует последние 5 матчей каждой команды:
...
📋 *После матчей используй /reports*
    """
    bot.reply_to(message, text, parse_mode='Markdown')

@bot.message_handler(commands=['reports'])
def send_reports(message):
    bot.reply_to(message, "📋 Собираю отчеты по завершенным матчам...")
    reports = []
    ...
    if not reports:
        bot.reply_to(message, "⏳ Нет завершенных матчей или прогнозов")
    else:
        for report in reports:
            bot.send_message(message.chat.id, report, parse_mode='Markdown')

def generate_predictions(league_filter=None):
    """ГЕНЕРИРУЕТ ПРОГНОЗЫ НА СЕГОДНЯ"""
    global predictions_db
    predictions_db = {}
        ...
        if matches:
            ...
            for match in matches:
                ...
                if prediction:
                    ...
                    report += f"   ⚽ *Тотал:* {prediction['total']}\n"
                    report += f"   🤝 *Обе забьют:* {prediction['btts']}\n"
                    report += f"   📊 *Форма:* {prediction['home_form']} | {prediction['away_form']}\n"
                    report += f"   🔑 `{prediction['match_id']}`\n\n"
            report += "---\n"
    
    if report == f"🔮 *ПРОГНОЗЫ НА {datetime.now().strftime('%d.%m.%Y')}*\n\n":
        report += "😴 Сегодня матчей нет или недостаточно данных."
    
    return report

@bot.message_handler(commands=['predict_all'])
def predict_all(message):
    bot.reply_to(message, "🔍 Анализирую все лиги... Это займет несколько секунд")
    report = generate_predictions()
    bot.send_message(message.chat.id, report, parse_mode='Markdown')

@bot.message_handler(commands=['predict_top5'])
def predict_top5(message):
    bot.reply_to(message, "🔍 Анализирую топ-5 лиг...")
...
def send_daily_predictions():
    ...
    for league_name, league_id in LEAGUES.items():
        ...
        for match in matches:
            ...
            if prediction:
                predictions_db[prediction['match_id']] = prediction
    print(f"✅ Прогнозы на {datetime.now().date()} сохранены. Всего: {len(predictions_db)}")
...
if __name__ == "__main__":
    print("=" * 50)
    print("⚽ ЗАПУСК БОТА @lyressports_bot")
    print("=" * 50)
    print(f"📅 Дата: {datetime.now().strftime('%d.%m.%Y')}")
    print(f"🏆 Сезон: {get_current_season()}")
    ...
    if test_data is not None:
        ...
        print("⚠️ Осторожно: API не отвечает, проверь ключ")
